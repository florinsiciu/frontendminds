...
import { aboutContentPillars } from "@/lib/content/about";
...
const accentMap: Record<string, { iconCircle: string; link: string }> = {
  purple: {
    iconCircle: "bg-[rgba(99,102,241,0.12)] text-[#818CF8]",
    link: "text-[#818CF8]",
  },
  indigo: {
    iconCircle: "bg-[rgba(94,234,212,0.12)] text-[#5EEAD4]",
    link: "text-[#5EEAD4]",
  },
  teal: {
    iconCircle: "bg-[rgba(251,191,36,0.12)] text-[#FBBF24]",
    link: "text-[#FBBF24]",
  },
  amber: {
    iconCircle: "bg-[rgba(196,181,253,0.12)] text-[#C4B5FD]",
    link: "text-[#C4B5FD]",
  },
};

export function AboutContentPillars() {
  return (
    <Section>
      <Reveal>
        <div className="mb-14 text-center">
          <Tagline>{aboutContentPillars.tagline}</Tagline>
          <h2 className="mb-5 font-heading text-[2rem] font-bold leading-[1.1] tracking-[-0.02em] text-foreground sm:text-[2.5rem]">
            {aboutContentPillars.headline}
          </h2>
          <p className="mx-auto max-w-[580px] text-[1.125rem] leading-[1.7] text-[#94A3B8]">
            {aboutContentPillars.description}
          </p>
        </div>
      </Reveal>
              ...
              <Link href={item.href} className="group block h-full">
                <GlassCard className="flex h-full items-start gap-4 p-5 sm:gap-6 sm:p-8">
                  ...
                  <div>
                    ...
                    <span
                      className={`mt-3 inline-block text-[0.8rem] font-semibold transition-colors ${accent.link}`}
                    >
                      Browse articles
                    </span>
                  </div>
